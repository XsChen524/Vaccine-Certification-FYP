...
template<typename FieldT>
sha256_multi_to_one_hash_gadget<FieldT>::sha256_multi_to_one_hash_gadget(protoboard<FieldT> &pb,
                                                                    const digest_variable<FieldT> &name,
                                                                    const digest_variable<FieldT> &hkid,
                                                                    const digest_variable<FieldT> &vaccineType,
                                                                    const digest_variable<FieldT> &location,
                                                                    const digest_variable<FieldT> &date,
                                                                    const digest_variable<FieldT> &sk,
                                                                    const digest_variable<FieldT> &rk,
                                                                    const digest_variable<FieldT> &output,
                                                                    const std::string &annotation_prefix) :
    gadget<FieldT>(pb, annotation_prefix)
{
    /* concatenate block = name || hkid || VaccineType || location || date || sk || rk */
    pb_variable_array<FieldT> block;
    block.insert(block.end(), name.bits.begin(), name.bits.end());
    block.insert(block.end(), hkid.bits.begin(), hkid.bits.end());
    block.insert(block.end(), vaccineType.bits.begin(), VaccineType.bits.end());
    block.insert(block.end(), location.bits.begin(), location.bits.end());
    block.insert(block.end(), date.bits.begin(), date.bits.end());
    block.insert(block.end(), sk.bits.begin(), sk.bits.end());
    block.insert(block.end(), rk.bits.begin(), rk.bits.end());
...
template<typename FieldT>
sha256_two_to_one_hash_gadget<FieldT>::sha256_multi_to_one_hash_gadget(protoboard<FieldT> &pb,
                                                                     const size_t block_length,
                                                                     const block_variable<FieldT> &input_block,
                                                                     const digest_variable<FieldT> &output,
                                                                     const std::string &annotation_prefix) :
...
template<typename FieldT>
void sha256_multi_to_one_hash_gadget<FieldT>::generate_r1cs_constraints(const bool ensure_output_bitness)
{
    libff::UNUSED(ensure_output_bitness);
    f->generate_r1cs_constraints();
}
...
template<typename FieldT>
void sha256_multi_to_one_hash_gadget<FieldT>::generate_r1cs_witness()
{
    f->generate_r1cs_witness();
}

template<typename FieldT>
size_t sha256_multi_to_one_hash_gadget<FieldT>::get_block_len()
{
    return SHA256_block_size;
}

template<typename FieldT>
size_t sha256_multi_to_one_hash_gadget<FieldT>::get_digest_len()
{
    return SHA256_digest_size;
}

template<typename FieldT>
libff::bit_vector sha256_multi_to_one_hash_gadget<FieldT>::get_hash(const libff::bit_vector &input)
{
    protoboard<FieldT> pb;

    block_variable<FieldT> input_variable(pb, SHA256_block_size, "input");
    digest_variable<FieldT> output_variable(pb, SHA256_digest_size, "output");
    sha256_multi_to_one_hash_gadget<FieldT> f(pb, SHA256_block_size, input_variable, output_variable, "f");
...
template<typename FieldT>
size_t sha256_multi_to_one_hash_gadget<FieldT>::expected_constraints(const bool ensure_output_bitness)
{
    libff::UNUSED(ensure_output_bitness);
    return 27280; /* hardcoded for now */
}
